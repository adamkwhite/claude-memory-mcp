...
class SearchBenchmark:
    ...
    def __init__(self, storage_path: str = "~/claude-memory-benchmark"):
        ...
        self.linear_server = ConversationMemoryServer(
            storage_path=str(self.storage_path / "linear"),
            enable_sqlite=False
        )
        
        self.sqlite_server = ConversationMemoryServer(
            storage_path=str(self.storage_path / "sqlite"), 
            enable_sqlite=True
        )
    ...
    def generate_test_data(self, num_conversations: int = 100) -> List[Dict[str, Any]]:
        ...
        tech_terms = [
            ...
            "authentication", "frontend", "backend", "testing", "deployment",
            "docker", "kubernetes", "aws", "performance", "optimization",
            "machine learning", "data science", "algorithms", "debugging"
        ]
    ...
    def run_search_benchmark(self, query: str, iterations: int = 10) -> Dict[str, Any]:
        ...
        return {
            ...
            "performance": {
                ...
                "percentage_improvement": ((linear_stats["mean"] - sqlite_stats["mean"]) / linear_stats["mean"] * 100) if linear_stats["mean"] > 0 else 0
            }
        }
    
    def run_comprehensive_benchmark(self, num_conversations: int = 100) -> Dict[str, Any]:
        """Run comprehensive benchmark with various queries."""
        self.logger.info(f"Running comprehensive benchmark with {num_conversations} conversations...")
        ...
        benchmark_results["overall"] = {
            ...
            "overall_speedup": overall_linear_mean / overall_sqlite_mean if overall_sqlite_mean > 0 else 0,
            "overall_improvement_percent": ((overall_linear_mean - overall_sqlite_mean) / overall_linear_mean * 100) if overall_linear_mean > 0 else 0
        }
