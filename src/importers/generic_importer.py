...
class GenericImporter(BaseImporter):
    ...
    def _parse_json_array(self, data: List[Any], file_path: Path) -> List[Dict[str, Any]]:
    ...
    def _parse_csv_rows(self, rows: List[Dict[str, str]], file_path: Path) -> Optional[Dict[str, Any]]:
    ...
    def _looks_like_conversation(self, data: Dict[str, Any]) -> bool:
        ...
        conversation_indicators = [
            "messages", "content", "dialogue", "conversation", "chat",
            "title", "participants", "transcript", "exchanges"
        ]
        
        text_indicators = [
            "text", "message", "response", "query", "answer"
        ]
    ...
    def _parse_message_blocks(self, content: str, file_path: Path = None) -> Dict[str, Any]:
    ...
    def _parse_text_as_conversation(self, content: str, file_path: Path = None) -> Dict[str, Any]:
    ...
    def _save_conversations(self, conversations: List[Dict[str, Any]], file_path: Path, format_type: str) -> ImportResult:
        ...
        for conv in conversations:
            try:
                if self._validate_conversation(conv):
                    ...
                    failed_count += 1
                    errors.append(f"Invalid conversation format for ID: {conv.get('id', 'unknown')}")
