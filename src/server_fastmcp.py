...
class FastMCPConversationMemoryServer(CoreMemoryServer):
    ...
    def __init__(self, storage_path: str = "~/claude-memory", use_data_dir: bool = None):
        ...
        log_function_call("FastMCPConversationMemoryServer.__init__", storage_path=storage_path, use_data_dir=use_data_dir)
        ...
        super().__init__(storage_path=storage_path, use_data_dir=use_data_dir, enable_sqlite=True)
        
        self.fastmcp_logger.info(f"FastMCP Server initialized with SQLite: {self.use_sqlite_search}")
    ...
    def _validate_storage_path(self, storage_path: Path):
        ...
        if '..' in str(storage_path):
            log_security_event("PATH_TRAVERSAL_ATTEMPT", f"Storage path contains '..' traversal: {storage_path}", "ERROR")
