...
class FormatDetector:
    ...
    def detect_format(self, file_path: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            if extension == '.json':
                ...
                return self._create_result(PlatformType.UNKNOWN, 0.0, f"Unsupported extension: {extension}")
    ...
    def _detect_json_format(self, file_path: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            if self._is_claude_memory_format(data):
                return self._create_result(PlatformType.CLAUDE_MEMORY, 0.99, "Claude Memory format detected")
            
            # Check for Claude Desktop format
            if self._is_claude_desktop_format(data):
                return self._create_result(PlatformType.CLAUDE_DESKTOP, 0.90, "Claude Desktop export detected")
            
            # Generic JSON with conversation-like structure
            if self._has_conversation_structure(data):
                return self._create_result(PlatformType.GENERIC_JSON, 0.60, "Generic conversation JSON detected")
    ...
    def _detect_text_format(self, file_path: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            if self._is_claude_web_format(content):
                return self._create_result(PlatformType.CLAUDE_WEB, 0.85, "Claude web conversation detected")
            
            # Check for generic markdown conversation
            if self._is_markdown_conversation(content):
                return self._create_result(PlatformType.GENERIC_MARKDOWN, 0.70, "Generic markdown conversation detected")
        ...
        if 'conversations' in data and isinstance(data['conversations'], list):
            ...
            if data['conversations']:
                ...
                return (isinstance(conv, dict) and 
                       'messages' in conv and 
                       isinstance(conv['messages'], list) and
                       self._has_role_based_messages(conv['messages']))
    ...
    def _is_claude_web_format(self, content: str) -> bool:
        ...
        pattern_matches = sum(1 for pattern in claude_patterns 
    ...
    def _is_markdown_conversation(self, content: str) -> bool:
        ...
        pattern_matches = sum(1 for pattern in conversation_patterns 
    ...
    def _has_conversation_structure(self, data: Any) -> bool:
        ...
        conversation_fields = ['messages', 'content', 'conversation', 'chat', 'dialogue']
    ...
    def _create_result(self, platform: PlatformType, confidence: float, message: str) -> Dict[str, Any]:
